...
void Cal_D_Val(){           // extensible to many grps also
	...
	int i,j ;
#pragma omp parallel for private(external, internal, i,j) schedule(dynamic)
	for(i=1; i<=n ; ++i){
		external = 0;
		internal = 0 ;
...
//	{
//#pragma omp for private(i) schedule(dynamic) nowait				// can do a section paralllelization but
	for(i=0 ; i < adj[a].size() ; ++i)				// wont be beneficial as number of cores is more
		a_edge[adj[a][i].first] = adj[a][i].second;


//#pragma omp for private(j) schedule(dynamic) nowait				// can do a section paralllelization but
	for(j=0 ; j<adj[b].size() ; ++j)
		b_edge[adj[b][j].first] = adj[b][j].second;

//#pragma omp barrier

//#pragma omp for private(k) schedule(dynamic) nowait				
...
//#pragma omp parallel for shared(maxg,a,b) private(i,j,k,Cab, newg) collapse(2) 
